...
<div class="container overflow-hidden w-fit md:px-16 py-5 rounded-3xl mx-4 sm:mx-auto -mt-32 bg-white z-10 relative shadow-2xl">
  ...
  <div class="flex overflow-x-auto gap-16">
    @if ($cooperationImg->isEmpty())
      <p class="text-center text-neutral-200 font-montserrat text-lg">
        Tidak ada gambar kerjasama yang tersedia.
      </p>
    @else
...
<section class="w-full overflow-hidden py-28">
  ...
  >
    <div class="space-y-4 lg:ml-20">
      ...
      @if (!isset($abouts->content) || $abouts->content === '' || !isset($abouts->image) || $abouts->image === '')
          <p class="font-montserrat text-lg font-semibold text-neutral-200 sm:text-xl">No content available</p>
      @else
        ...
        <p
          class="text-base sm:text-lg font-semibold text-neutral-200 font-montserrat"
        >
          {{ $abouts->content  }}
        </p>
        <a
...
<section class="container relative">
  <div class="mb-10 flex justify-between items-center">
    <div>
      <h3
        class="font-semibold text-xl sm:text-2xl font-montserrat text-neutral-400"
      >
        Berita terkini untuk Anda
      </h3>
      <p
        class="text-sm sm:text-base font-montserrat text-neutral-200 font-semibold"
      >
    ...
    <div>
      <button
        class="text-neutral-400 hover:text-neutral-0 bg-neutral-0 transition-all hover:bg-neutral-400 w-10 h-10 rounded-full"
      >
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    @if ($news->empty())
      <p class="font-montserrat text-lg font-semibold text-neutral-200 sm:text-xl">No news available</p>
    @else
      @foreach ($news as $newslist)
      <div class="p-[14px] rounded-[20px] border border-neutral-100 bg-neutral-0">
        ...
        <a
          ...
          class="text-base sm:text-lg font-montserrat font-semibold text-neutral-400 line-clamp-2"
        >
          {{ $newslist->title }}
        </a>
        <p
          class="font-montserrat text-xs sm:text-sm font-semibold text-neutral-200"
        >
...
<section class="container mt-28">
  <div class="text-center space-y-2">
    <h3
      class="font-montserrat font-semibold text-neutral-400 text-xl sm:text-2xl"
    >
      Rektorat B-Universitas
    </h3>
    <p
      class="font-montserrat text-sm sm:text-base font-semibold text-neutral-200"
    >
  ...
  <div class="grid grid-cols-2 gap-12 lg:grid-cols-4 mt-11 text-center">
    @if ($rectors->isEmpty())
      <p class="font-montserrat text-lg font-semibold text-neutral-200 sm:text-xl">No rectors available</p>
    @else
      @foreach ($rectors as $rektor)
        <div class="flex flex-col items-center">
          ...
          <p
            class="mb-[2px] text-sm sm:text-base text-neutral-400 font-semibold font-montserrat"
          >
            {{ $rektor->nama }}
          </p>
          <p
            class="mb-[2px] text-xs sm:text-sm text-neutral-200 font-semibold font-montserrat"
          >
            {{ $rektor->jabatan }}
          </p>
        </div> 
...
<section class="mt-28 x-announcement w-full">
  <div class="container pt-9 pb-16">
    <div class="mb-10 flex justify-between items-center">
      <div>
        <h3
          class="font-semibold text-xl sm:text-2xl font-montserrat text-neutral-0"
        >
          Pengumuman
        </h3>
        <p
          class="font-montserrat text-sm sm:text-base text-neutral-0 font-semibold"
        >
      ...
      @forelse ($announcements as $announcement)
        <div class="py-[26px] px-7 rounded-[20px] border border-neutral-100 bg-white">
          <a
            ...
            class="text-base sm:text-lg font-montserrat font-semibold text-neutral-400 line-clamp-2 mb-4"
          >
            {{ $announcement->title }}
          </a>
          <p
            class="font-montserrat text-xs sm:text-sm font-semibold text-neutral-200 mb-1.5"
          >
        ...
        @empty
        <div class="py-[26px] px-7 rounded-[20px] border border-neutral-100 bg-white">
            <p class="font-montserrat text-lg font-semibold text-neutral-200 sm:text-xl">No announcements available</p>
        </div>
      @endforelse
    </div>
  </div>
